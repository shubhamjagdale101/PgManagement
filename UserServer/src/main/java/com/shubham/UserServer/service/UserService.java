...
public class UserService implements UserDetailsService {
    ...
    public User handleSignUp(SignUp req, String role) throws JsonProcessingException {
...
//        kafkaTemplate.send(UserCreatedConstant.USER_CREATED, objectMapper.writeValueAsString(jsonObject.toString()));
    ...
    public ResponseEntity<Object> availFacility(String serviceName) throws Exception {
        ...
        if(usersFacilities != null) throw new Exception("facility is already in use");
    ...
    public ResponseEntity<String> applyToLeave() throws Exception {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        user = userRepository.findByUserName(user.getUsername());
        ...
        return new ResponseEntity<>("applied for to leave", HttpStatus.ACCEPTED);
    }

    public ResponseEntity<String> messageToAdmin(MessageRequest messageRequest){
        ...
        return new ResponseEntity<>("message send successfully", HttpStatus.ACCEPTED);
    }
